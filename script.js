      ...
      if (result.info.moderation[0].status == 'approved') {
        ...
        if (format == 'jpg' || format == 'png') {
          ...
          var node = document.createTextNode(
            'Delivery URL: ' + result.info.secure_url
          );
          ...
          const objects = [];
          const tagsObject =
            result.info.info.detection.object_detection.data.unidet.tags;
          const tagNames = Object.keys(tagsObject);

          for (let i = 0; i < tagNames.length; i++) {
            if (
              tagNames[i] != 'furniture' &&
              tagNames[i] != 'girl' &&
              tagNames[i] != 'spectacles' &&
              tagNames[i] != 'necktie' &&
              tagNames[i] != 'man' &&
              tagNames[i] != 'woman' &&
              tagNames[i] != 'boy' &&
              tagNames[i] != 'human-face' &&
              tagNames[i] != 'person'
            ) {
          ...
          if (objects.length == 0) {
            alert(
              "We couldn't detect any objects. Try uploading a new file that contains more objects."
            );
            window.location.reload();
          } else {
            var intro = document.createElement('h3');
            intro.textContent =
              'Our content aware add-on identified these objects in your image:';
          ...
          if (objects.length == 1) {
            alert(
              'We only detected one object. Play around with recolor, then try uploading a new file that contains more objects.'
            );
          }
        } else {
